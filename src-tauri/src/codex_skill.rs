...
const SKILL_NAME: &str = "baijimu-docs";
const SKILL_FILE_NAME: &str = "SKILL.md";
const MAX_SKILL_BYTES: usize = 256 * 1024;
const BUNDLED_SKILL: &[u8] = include_bytes!("../resources/skills/baijimu-docs/SKILL.md");

pub fn install_bundled() -> Result<PathBuf> {
    validate_skill(BUNDLED_SKILL)?;
    install(BUNDLED_SKILL, &skill_path()?)
}
...
fn skill_path() -> Result<PathBuf> {
    if let Some(root) = std::env::var_os("BAIJIMU_CODEX_SKILLS_DIR") {
        return Ok(PathBuf::from(root).join(SKILL_NAME).join(SKILL_FILE_NAME));
    }
    Ok(dirs::home_dir()
...
mod tests {
    ...
    fn installs_bundled_skill_in_user_skill_directory() {
        ...
        std::env::set_var("BAIJIMU_CODEX_SKILLS_DIR", temp.path());

        let installed = install_bundled().unwrap();
        assert_eq!(installed, temp.path().join("baijimu-docs").join("SKILL.md"));
        assert_eq!(fs::read(&installed).unwrap(), BUNDLED_SKILL);

        std::env::remove_var("BAIJIMU_CODEX_SKILLS_DIR");
    }
