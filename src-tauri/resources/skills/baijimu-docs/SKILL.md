...
1. 运行 `baijimu --version`，确认本机 CLI 版本。如果命令不存在，不要直接判断 CLI 未安装：
   - Windows：检查并直接运行 `$env:LOCALAPPDATA\Baijimu\bin\baijimu.exe`；这通常表示 Codex 在 Bridge Agent 首次配置用户 PATH 前已经启动，后续命令继续使用该绝对路径，并提示用户完成当前任务后重启 Codex。
   - macOS/Linux：检查并直接运行 `~/.local/bin/baijimu`。
2. 使用上一步确认可用的命令入口运行 `capabilities --help`。如果支持 `--offline`，运行 `capabilities --offline --json`。
...
5. 涉及具体命令时，先使用已确认的命令入口运行 `<command> --help`，以本机已安装版本的帮助为准，不编造命令、参数、资源 ID 或 Bundle 格式。
6. 需要账号或工作区动态资源时，先使用已确认的命令入口运行 `auth status --verify`；未登录时运行 `auth login` 并让用户在浏览器完成授权。
