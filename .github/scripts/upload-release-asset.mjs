...
if (!isAllowedReleaseAsset(assetName, target)) {
  throw new Error(
    `Refusing to upload non-release bundle for ${target}: ${assetName}`,
  );
}
...
const filesToUpload = [
  { name: assetName, path: filePath, type: contentType, size },
];
if (signaturePath) {
...
for (const file of filesToUpload) {
  ...
  let uploaded = await findReusableAttachment(matchingAttachments, file);
  if (uploaded) {
    console.log(`Reused verified ${file.name} from Gitee Release`);
  } else {
    ...
    uploaded = await uploadGiteeAttachment(releaseId, file);
...
const downloadUrl =
  uploadedAsset?.browser_download_url ?? uploadedAsset?.download_url;
if (!downloadUrl) {
  throw new Error(
    `Gitee did not return a public download URL for ${assetName}`,
  );
}
    ...
    async () => {
      try {
        ...
        return decodeJsonResponse(
          response,
          `upload Gitee attachment ${file.name}`,
        );
      } catch (error) {
        const currentAttachments = await giteeJson(
          `/repos/${owner}/${repository}/releases/${releaseId}/attach_files`,
          { query: { page: "1", per_page: "100" } },
        );
        const recovered = await findReusableAttachment(
        ...
        if (recovered) {
          console.warn(
            `Recovered ${file.name} from Gitee after upload response failure: ${error.message}`,
          );
          return recovered;
        }
        throw error;
...
async function findReusableAttachment(candidates, file) {
  for (const candidate of candidates) {
    const downloadUrl =
      candidate.browser_download_url ?? candidate.download_url;
    if (!candidate.id || !downloadUrl) {
      continue;
    }
    try {
      validateGiteeDownloadUrl(downloadUrl, file.name);
      await verifyPublicDownload(downloadUrl, file.size, file.name);
      return candidate;
    } catch (error) {
      console.warn(
        `Cannot reuse Gitee attachment ${file.name}: ${error.message}`,
      );
    }
  }
  return undefined;
...
async function verifyPublicDownload(url, expectedSize, name) {
  await retry(
    async () => {
      ...
      try {
        if (response.status !== 200 && response.status !== 206) {
          throw new Error(
            `anonymous download returned HTTP ${response.status}`,
          );
        }
        ...
        if (contentRange) {
          ...
          if (match && Number(match[1]) !== expectedSize) {
            throw new Error(
              `content-range size ${match[1]} does not match ${expectedSize}`,
            );
          }
        } else if (contentLength && Number(contentLength) !== expectedSize) {
          throw new Error(
            `content-length ${contentLength} does not match ${expectedSize}`,
          );
        }
      } finally {
    ...
    `public download verification for ${name}`,
    6,
    10_000,
  );
...
function validateGiteeDownloadUrl(value, expectedName = assetName) {
  ...
  if (
    ...
    throw new Error(
      `Gitee returned a non-permanent download URL for ${expectedName}`,
    );
  }
  const prefix = `/${owner}/${repository}/`;
  if (!url.pathname.startsWith(prefix)) {
    throw new Error(
      `Gitee download URL for ${expectedName} does not belong to ${owner}/${repository}`,
    );
  }
}
