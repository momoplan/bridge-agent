...
| --- | --- | --- |
| ChatGPT desktop app | OpenAI 官方桌面应用。 | 用户使用 Codex 设备安装能力时的首选安装对象。由平台 Agent 通过百积木的 `shellExec.shellExec` 自动安装，用户不手工下载安装包。 |
...
| relay `shellExec` | 通过已连接的百积木在用户设备上执行安装和验证命令。 | 是普通用户自助安装 ChatGPT desktop app/终端能力的标准执行通道，但必须经过用户授权和 allowlist 控制。 |
...
5. 系统自动搜索并加载公开市场中的 Codex 设备安装能力，通过 `shellExec.shellExec` 完成 ChatGPT desktop app 安装和验证。
...
| 安装 ChatGPT desktop app | AI 通过当前设备暴露的 `shellExec.shellExec` 方法执行官方 ChatGPT desktop app 安装、启动和验证命令。 | bridge-agent/relay/安装技能 |
...
4. 在用户明确授权后，通过 `shellExec.shellExec` 安装 ChatGPT desktop app，并执行启动和安装验证。
...
推荐路径是用户打开百积木完成设备授权，然后在百积木首页点击“打开控制台”，进入百积木控制台并打开 Agent 对话页面，输入“请使用 Codex 设备安装能力，给当前工作区的设备安装 Codex。”系统会自动搜索并会话内加载公开市场技能，再通过 `shellExec.shellExec` 完成 ChatGPT desktop app 安装、启动和验证。百积木当前没有内置对话输入框，“打开控制台”按钮会打开平台控制台入口。
...
现有 Codex 设备安装能力应作为平台市场技能使用：用户不需要手动安装技能，也不需要把技能预装到默认助理；Agent 应在会话中搜索、加载并执行该能力。技能优先通过 `shellExec.shellExec` 安装 ChatGPT desktop app；需要终端能力时，再创建用户专属凭证、配置 router 并验证计量归属。
...
| ChatGPT desktop app 安装失败 | `shellExec` 授权、allowlist、官方安装器下载、系统安装策略 | 先确认百积木权限，再看企业终端管控或系统安装提示。 |
