...
实施 SOP 的目标是完成企业初始用户的组织开通、设备授权、ChatGPT desktop app 自动安装、Codex 终端能力可选开通、端到端验证和交付验收。实施人员不应手工为用户生成 Codex 密钥；标准流程应由平台后台、Agent 对话安装流程和百积木本机命令能力自动完成安装、凭证生成、模型绑定、router 配置和校验。
...
| --- | --- | --- |
| ChatGPT desktop app | OpenAI 官方桌面应用。 | 用户使用 Codex 设备安装能力时的首选安装对象。由平台 Agent 通过百积木的 `shell.exec` 或 `shell.startExecution` 自动安装，用户不手工下载安装包。 |
...
| lowcode-apikey | 底层用户凭证和系统模型密钥绑定服务。 | 对普通用户不可见。只在配置 Codex 终端能力时由平台自动开通流程调用，或由运维在异常排查时查看脱敏绑定状态。ChatGPT desktop app 安装本身不需要调用它。 |
| relay `shell` | 通过已连接的百积木在用户设备上执行安装和验证命令。 | 是普通用户自助安装 ChatGPT desktop app/终端能力的标准执行通道，但必须经过用户授权和 allowlist 控制。 |
...
5. 系统自动搜索并加载公开市场中的 Codex 设备安装能力，通过 `shell` 服务完成 ChatGPT desktop app 安装和验证。
6. 如果企业需要 Codex 终端能力，系统继续自动完成凭证写入、router 配置、启动和健康检查。
7. 用户在对话中输入“确认 Codex 是否安装成功”完成首次验证。

因此，App 安装命令、CLI、router、lowcode-apikey 和 relay `shell` 都应该被产品化流程封装起来。普通用户只看到 百积木、设备授权、平台 Agent 对话和 Codex 安装状态。
...
| 安装 ChatGPT desktop app | AI 通过当前设备暴露的 `shell.exec` 或 `shell.startExecution` 执行官方 ChatGPT desktop app 安装、启动和验证命令。 | bridge-agent/relay/安装技能 |
...
| 设备执行配置 | AI 通过当前设备暴露的 `shell.exec` 或 `shell.startExecution` 执行配置写入、启动和健康检查。 | bridge-agent/relay |
| 结果校验 | 安装后校验 ChatGPT desktop app 状态和可见窗口；需要终端能力时，再通过 router `/responses`、凭证、计量日志确认归属一致。 | 安装工具链/平台日志 |
...
- `shell.exec` / `shell.startExecution` 没有授权给当前对话 agent，或命令 allowlist 不允许安装命令。
- 安装工具链还没有把 ChatGPT desktop app 安装、router、凭证和终端配置写成一个幂等流程。

### 0.2 平台技能与 `shell` 权限边界
...
3. 检查当前设备是否已暴露 `shell.exec`、`shell.startExecution` 和 `shell.queryExecution`，以及安装所需命令是否在 allowlist 内。
4. 在用户明确授权后，通过 `shell` 服务安装 ChatGPT desktop app，并执行启动和安装验证。长时间任务使用 `shell.startExecution` 并通过 `shell.queryExecution` 轮询。
...
7. 重启或打开 ChatGPT desktop app，并确认主进程、app-server 和可见窗口都存在。
8. 返回安装结果；涉及终端能力时，在平台侧校验 router、凭证、计量和审计归属。

`shell` 是本机命令执行服务，必须按用户授权和 allowlist 运行。安装技能不应尝试绕过权限。如果当前 Agent 没有执行权限，或者 allowlist 不允许安装命令，技能应该明确提示用户在百积木中开启对应本机能力，并说明这次授权用于安装和验证 Codex。

权限引导应包含：

- 需要开启的本机方法：`shell.exec`、`shell.startExecution` 和 `shell.queryExecution`。
- 需要允许的命令范围：ChatGPT desktop app 官方安装器下载/启动、OSS fallback 下载、SHA256 校验、应用安装/启动、可见窗口验证；如果需要终端能力，再包括终端配置写入、版本检查、router `/responses` 健康检查。
...
推荐路径是用户打开百积木完成设备授权，然后在百积木首页点击“打开控制台”，进入百积木控制台并打开 Agent 对话页面，输入“请使用 Codex 设备安装能力，给当前工作区的设备安装 Codex。”系统会自动搜索并会话内加载公开市场技能，再通过 `shell` 服务完成 ChatGPT desktop app 安装、启动和验证。百积木当前没有内置对话输入框，“打开控制台”按钮会打开平台控制台入口。
...
- ChatGPT desktop app 已安装、可启动，并且有可见窗口。仅有进程或菜单栏不算完成。
...
现有 Codex 设备安装能力应作为平台市场技能使用：用户不需要手动安装技能，也不需要把技能预装到默认助理；Agent 应在会话中搜索、加载并执行该能力。技能优先通过 `shell` 服务安装 ChatGPT desktop app；需要终端能力时，再创建用户专属凭证、配置 router 并验证计量归属。
...
这是试点或排障路径。企业版普通用户的标准入口是平台 Agent 对话；当前标准安装对象包括 ChatGPT desktop app，终端能力按企业产品流按需启用。
...
| ChatGPT desktop app 安装失败 | `shell` 授权、allowlist、官方安装器下载、系统安装策略 | 先确认百积木权限，再看企业终端管控或系统安装提示。 |
| 官方下载失败 | 用户网络到 GitHub、ChatGPT、oaistatic 的连通性；OSS fallback manifest | 切换百积木 OSS 官方资产缓存，校验 SHA256 后安装，不使用第三方镜像。 |
| ChatGPT desktop app 启动后无窗口 | App 主进程、app-server、`lsappinfo windows`、桌面截图 | 清理残留进程后重开；如果 `windows=[ NULL ]`，记录为 App 窗口创建问题，不算安装完成。 |
...
- 设备已完成授权并在线。
- ChatGPT desktop app 已通过对话安装流程安装并可启动。
- ChatGPT desktop app 已打开并显示可见窗口；如果需要登录、授权或系统权限，已记录具体阻塞点。
