...
实施 SOP 的目标是完成企业初始用户的组织开通、设备授权、Codex App 自动安装、Codex 终端能力可选开通、端到端验证和交付验收。实施人员不应手工为用户生成 Codex 密钥；标准流程应由平台后台、Agent 对话安装流程和 Bridge Agent 本机命令能力自动完成安装、凭证生成、模型绑定、router 配置和校验。
...
| --- | --- | --- |
| Codex App | OpenAI 官方桌面应用。 | 用户使用 Codex 设备安装能力时的首选安装对象。由平台 Agent 通过 Bridge Agent 的 `shellExec.shellExec` 自动安装，用户不手工下载安装包。 |
...
| relay `shellExec` | 通过已连接的 Bridge Agent 在用户设备上执行安装和验证命令。 | 是普通用户自助安装 Codex App/终端能力的标准执行通道，但必须经过用户授权和 allowlist 控制。 |

用户自主安装的目标流程应是：

1. 用户安装并打开 bridge-agent。
2. 用户完成企业账号授权。
3. Bridge Agent 获得设备授权上下文和本机能力授权。
4. 授权完成后，用户在 Bridge Agent 首页点击“打开控制台”，进入百积木控制台并打开 Agent 对话页面，输入“请使用 Codex 设备安装能力，给当前工作区的设备安装 Codex。”
...
因此，App 安装命令、CLI、router、lowcode-apikey 和 relay `shellExec` 都应该被产品化流程封装起来。普通用户只看到 Bridge Agent、设备授权、平台 Agent 对话和 Codex 安装状态。
...
“Codex 设备安装能力”应该沉淀为平台市场技能或平台工具链能力，而不是写死在 Bridge Agent 前端，也不是让普通用户照着下载页或 CLI 文档手工执行。平台技能负责把用户意图编排成可审计的自动流程：
...
`shellExec.shellExec` 是本机命令执行能力，必须按用户授权和 allowlist 运行。安装技能不应尝试绕过权限。如果当前 Agent 没有执行权限，或者 allowlist 不允许安装命令，技能应该明确提示用户在 Bridge Agent 中开启对应本机能力，并说明这次授权用于安装和验证 Codex。
...
6. 确认 Bridge Agent 公共下载页、百积木平台 Agent 对话页和公开市场中的 Codex 设备安装能力可用。
...
工作区用户侧动作详见 [Codex 工作区用户安装与使用指南](./codex-user-quickstart.md)。个人用户只需要连接设备时，使用 [Bridge Agent 个人用户安装指南](./bridge-agent-user-install.md)。

实施人员需要确认：

1. 用户从 Bridge Agent 公共下载页获取正确安装包。
...
推荐路径是用户打开 Bridge Agent 完成设备授权，然后在 Bridge Agent 首页点击“打开控制台”，进入百积木控制台并打开 Agent 对话页面，输入“请使用 Codex 设备安装能力，给当前工作区的设备安装 Codex。”系统会自动搜索并会话内加载公开市场技能，再通过 `shellExec.shellExec` 完成 Codex App 安装、启动和验证。Bridge Agent 当前没有内置对话输入框，“打开控制台”按钮会打开平台控制台入口。

如果企业环境还需要百积木 router-backed 的 Codex 终端能力，用户或管理员可以继续输入“安装 Codex 终端”，由同一技能自动完成终端工具安装、凭证写入、router 配置和健康检查。

实施人员需要确认：

- 用户授权后可以在 Bridge Agent 首页点击“打开控制台”，并在百积木控制台进入 Agent 对话页面。
...
| Codex App 安装失败 | `shellExec` 授权、allowlist、官方安装器下载、系统安装策略 | 先确认 Bridge Agent 权限，再看企业终端管控或系统安装提示。 |
