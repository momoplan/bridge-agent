...
- Windows：在百积木“诊断 → 系统”中点击“卸载百积木”。卸载向导会明确询问是保留设备配置和本地应用数据，还是完全删除配置、凭证、Connector、托管 Baijimu CLI、用户 PATH 项及相关本机数据。直接从 Windows“已安装的应用”卸载时会清理进程、登录启动、缓存和临时状态，但默认保留可恢复的业务数据。
- macOS：退出百积木后删除“应用程序”中的“百积木.app”；当前 DMG 遵循 macOS 拖拽安装惯例，不提供 Windows 式卸载器。
- Linux：`.deb` 使用系统包管理器卸载；`.AppImage` 退出后删除文件。

Windows 完全卸载只删除百积木管理的默认目录。用户在高级设置中指定的任意外部日志目录不会被递归删除，避免误删其他业务文件。
...
该命令会构建前端，随后执行所有 Rust、TypeScript/JavaScript、CSS、脚本和 CI 源文件的长度硬门禁，
以及 Rust 文件/函数长度与认知复杂度债务棘轮、全部 Rust 测试、Clippy 和桌面端 release check。
Pull Request 和 `release-bridge-agent` 打包流水线都调用这个入口。阈值配置位于
`config/rust-architecture.json`，存量债务位于 `config/rust-architecture-baseline.json`；当前 baseline
为空，后续禁止新增或提高。通用规范见
