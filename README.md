...
- Windows：在百积木“诊断 → 系统”中点击“卸载百积木”，或从开始菜单打开“卸载百积木”。卸载向导会明确询问是保留设备配置和本地应用数据，还是完全删除配置、凭证、Connector、托管 Baijimu CLI、用户 PATH 项及相关本机数据。直接从 Windows“已安装的应用”卸载时会清理进程、登录启动、缓存和临时状态，但默认保留可恢复的业务数据。
