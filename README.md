...
3. GitHub Actions 负责构建、签名、公证和生成安装包
4. 发布流程向平台 release service 申请 OSS 预签名上传地址
5. GitHub Actions 把安装包直接上传到国内 OSS/CDN，并回写对象 key 与 sha256
6. 所有平台产物上传完成后，发布流程调用 release service 的 publish 接口
7. Jenkins 校验 GitHub 四类安装包和公开更新接口中的最终版本
8. 客户端只请求 `BRIDGE_AGENT_UPDATE_API_URL` 判断是否有新版本，并从服务返回的国内下载地址下载安装包
