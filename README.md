...
- `APPLE_API_PRIVATE_KEY`
  - 下载得到的 `.p8` 私钥全文内容
- `AZURE_CLIENT_ID`
  - 用于 Windows Artifact Signing 的 Microsoft Entra App Registration client ID
- `AZURE_CLIENT_SECRET`
  - 上面 App Registration 的 client secret value
- `AZURE_TENANT_ID`
  - Azure tenant ID
- `AZURE_ARTIFACT_SIGNING_ENDPOINT`
  - Artifact Signing account 所在区域的 endpoint，例如 `https://eus.codesigning.azure.net`
- `AZURE_ARTIFACT_SIGNING_ACCOUNT`
  - Artifact Signing account 名称
- `AZURE_ARTIFACT_SIGNING_CERTIFICATE_PROFILE`
  - Public Trust Code Signing certificate profile 名称
...
1. 在 Azure 订阅里注册 `Microsoft.CodeSigning` resource provider
2. 创建 Basic SKU 的 Artifact Signing account
3. 在 Artifact Signing account 里完成 Organization 或 Individual identity validation
4. 创建 Public Trust 的 Code Signing certificate profile
5. 创建 Microsoft Entra App Registration，并生成 client secret
6. 给该 App Registration 分配 `Artifact Signing Certificate Profile Signer` 角色
7. 把上面的 Azure 信息写入 GitHub Secrets
